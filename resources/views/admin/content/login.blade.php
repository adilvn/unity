<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Login - Unity Admin Dashboard</title>
    <link href="https://fonts.googleapis.com/css2?family=Nunito:wght@300;400;600;700;800&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="{{ asset('adminAssets/css/bootstrap.css')}}">
    <link rel="stylesheet" href="{{ asset('adminAssets/vendors/bootstrap-icons/bootstrap-icons.css')}}">
    <link rel="stylesheet" href="{{ asset('adminAssets/css/app.css')}}">
    <link rel="stylesheet" href="{{ asset('adminAssets/css/pages/auth.css')}}">
    <link rel="stylesheet" href="{{ asset('adminAssets/css/toastr.min.css')}}">
    <script src="{{ asset('adminAssets/js/jquery.min.js')}}"></script>
    <script src="{{ asset('adminAssets/js/toastr.min.js')}}"></script>
</head>

<body>
    <div id="auth">

        <div class="row h-100">
            <div class="col-lg-5 col-12">
                <div id="auth-left">
                    <div class="auth-logo">
                        <a href="#"><img src="{{ asset('adminAssets/images/logo/unity-logo.png')}}" style="height:auto" alt="Logo"></a>
                    </div>
                    <h1 class="auth-title">Log in.</h1>
                    <p class="auth-subtitle mb-5">Login to Unity Admin Dashboard</p>

                    <form action="{{ route('admin-login') }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <div class="form-group position-relative has-icon-left mb-0">
                                <input type="email" name="email" value="{{old('email')}}" class="form-control form-control-xl" placeholder="Email">
                                <div class="form-control-icon">
                                    <i class="bi bi-person"></i>
                                </div>
                            </div>
                            <span class="text-danger small">
                                @error('email')
                                    Admin Email is Required!
                                @enderror
                            </span>
                        </div>
                        <div class="mb-4">
                            <div class="form-group position-relative has-icon-left mb-0">
                                <input type="password" name="password" class="form-control form-control-xl" placeholder="Password">
                                <div class="form-control-icon">
                                    <i class="bi bi-shield-lock"></i>
                                </div>
                            </div>
                            <span class="text-danger small">
                                @error('password')
                                    Admin Password is Required!
                                @enderror
                            </span>
                        </div>
                        <div class="form-check form-check-lg d-flex align-items-end">
                            <input class="form-check-input me-2" name="remember" type="checkbox" id="remember">
                            <label class="form-check-label text-gray-600" for="remember">
                                Keep Me Logged-in
                            </label>
                        </div>
                        <button class="btn btn-primary btn-block btn-lg shadow-lg mt-5">Log in</button>
                    </form>
                </div>
            </div>
            <div class="col-lg-7 d-none d-lg-block">
                <div id="auth-right">

                </div>
            </div>
        </div>

    </div>
    <script>
        $(function() {
            @if (Session::has('admin-login-error'))
                toastr.options =
                {
                    "timeOut": "3000",
                    "closeButton": true,
                    "progressBar": true
                }
                toastr.error("{{ Session::get('admin-login-error') }}");
            @endif
            @if (Session::has('superadmin-not-login'))
                toastr.options =
                {
                    "timeOut": "3000",
                    "closeButton": true,
                    "progressBar": true
                }
                toastr.error("{{ Session::get('superadmin-not-login') }}");
            @endif
            @if (Session::has('admin-not-login'))
                toastr.options =
                {
                    "timeOut": "3000",
                    "closeButton": true,
                    "progressBar": true
                }
                toastr.error("{{ Session::get('admin-not-login') }}");
            @endif
        });
    </script>
</body>

</html>
